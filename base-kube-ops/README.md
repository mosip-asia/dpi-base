...
* **Setup order** (verified against Rancher 2.15.1):
  ...
  2. Open Rancher on the server URL (not the base name) and configure the Generic OIDC provider there; the form pre-fills the redirect URL from the address bar. Choose **Specify** for the endpoints (the **Generate** option builds Keycloak-style paths from a URL and a realm) and enter Google's published OIDC endpoints: Issuer `https://accounts.google.com`, Auth `https://accounts.google.com/o/oauth2/v2/auth`, Token `https://oauth2.googleapis.com/token`, User Info `https://openidconnect.googleapis.com/v1/userinfo`, JWKS `https://www.googleapis.com/oauth2/v3/certs`; scopes `openid profile email`.
